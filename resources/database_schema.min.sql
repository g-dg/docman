...
CREATE TABLE IF NOT EXISTS "files"("id" INTEGER PRIMARY KEY NOT NULL,"mountpoint_id" INTEGER NOT NULL REFERENCES "mountpoints","path_in_mountpoint" TEXT NOT NULL,"display_name" TEXT"type" INTEGER NOT NULL DEFAULT 0,"mountpoint_driver_info" TEXT NOT NULL,UNIQUE("mountpoint_id", "path_in_mountpoint"));
CREATE TABLE IF NOT EXISTS "file_permissions"("id" INTEGER PRIMARY KEY NOT NULL,"file_id" INTEGER NOT NULL REFERENCES "files","group_id" INTEGER NOT NULL REFERENCES "groups","owner_user_id" INTEGER REFERENCES "users","read" INTEGER NOT NULL,"write" INTEGER NOT NULL,"share" INTEGER NOT NULL,"expires" INTEGER);
...
CREATE TABLE IF NOT EXISTS "file_history"("id" INTEGER PRIMARY KEY,"user_id" INTEGER NOT NULL REFERENCES "users","file_path" TEXT NOT NULL,"timestamp" INTEGER NOT NULL DEFAULT(STRFTIME('%s', 'now')),"action" TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS "favourites"("id" INTEGER PRIMARY KEY,"user_id" INTEGER NOT NULL REFERENCES "users","file_path" TEXT NOT NULL,"sort_order" INTEGER NOT NULL);
CREATE TABLE IF NOT EXISTS "search_index"("id" INTEGER PRIMARY KEY NOT NULL,"file_path" TEXT NOT NULL,"keyword" TEXT NOT NULL,"type" TEXT,"mtime" INTEGER,"size" INTEGER,"last_indexed" INTEGER NOT NULL DEFAULT(STRFTIME('%s', 'now')));
